...
// ─── QR PAYLOAD HELPERS ─────────────────────────────────
// QR codes generated by the seed/queue-create flow contain a JSON object:
//   { type: 'QUEUE_JOIN', queueId, queueName, prefix, timestamp }
// We also accept a bare queue id (cuid) so manual codes work too.
function parseQrPayload(raw: string): { queueId: string; queueName?: string } | null {
  if (!raw) return null
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && parsed.queueId) {
      return { queueId: String(parsed.queueId), queueName: parsed.queueName }
    }
  } catch {
    /* not JSON — fall through */
  }

  // Treat as bare queue id (cuid-ish strings of 20-32 chars)
  if (/^[a-zA-Z0-9_-]{8,40}$/.test(trimmed)) {
    return { queueId: trimmed }
  }
...
export function QRScannerScreen() {
  ...
  const decodeImageFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const parsed = parseQrPayload(code.data)
      if (!parsed) {
        setError('That QR code is not a QueueSeva queue code.')
        return
      }
      const queueName = await hydrateQueueName(parsed.queueId, parsed.queueName)
      setScannedResult({ queueId: parsed.queueId, queueName })
  ...
  const handleManualJoin = async () => {
    ...
    const queueId = parsed?.queueId ?? manualCode.trim()
    try {
      const result = await apiClient.getQueue(queueId)
      if (result.success && result.data) {
